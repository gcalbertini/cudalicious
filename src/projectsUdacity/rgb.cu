                                  ...
                                  int numRows, int numCols)
{
  ...
  */
  ...
  if (row < numRows && col < numCols)
  {
    ...
    int offset = row * numCols + col; // row-major order
                            ...
                            unsigned char *const d_greyImage, size_t numRows, size_t numCols)
{
  ...
  const dim3 blockSize(threadX, threadY, 1); // Set 16x16 to start; powers of 2 for efficiency of mem access

  // if the input dimensions are not exact multiples of the block size, adjust the grid dimensions to cover the entire input
  // if we knew the T total threads in a dim and the V threads/block in that dim then min blocks is (V+T-1)/V to cover image
  const dim3 gridSize((numCols / threadX) + 1, (numRows / threadY) + 1);
